...
TEST(TestStop, InitStop) {
  ...
  const auto test_stop = std::make_shared<const Stop>("Test Stop", test_line);
...
TEST(TestStop, Transfers) {
  ...
  const auto transfer1 = std::make_shared<Stop>("Transfer 1", test_line);
  test_stop->transfers.insert(transfer1);
  const auto transfer2 = std::make_shared<Stop>("Transfer 2", test_line);
